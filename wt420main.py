...
database = databasepath + "weight.db"
...
def getStats(x):
    ...
    outputString += "Difference: " + "{:.1f}".format((weights[0] - weights[1])) + "kg.\n"

    diff =  "{:.2f}".format(float(weights[0] - weights[1])/float(x))
    outputString += "Average difference per day: " + str(diff) + "kg for " + str(x) + " days.\n"
...
def fillEmptyDays():
    ...
    yesterday = str((today - datetime.timedelta(days=1)).date())
    today = str(today.date())
    #print(today)
    #print(yesterday)
    if lDate != yesterday and lDate != today:
        print("Filling in empty days...\n")
        while lDate != yesterday:
